...
export const handlePrompt = async (
    ...
    if (providerResponse && bot.type === 'chat_continuous' && !conversation.name) {
        ...
        const generatedTitle = await getProviderResponse(provider.id, rapidPayload)
            .catch(() => {
            }) as string || inputText;
...
const getProviderResponse = async (
    providerId: string,
...
export const callProviderHandler = async (
    providerId: string,
    payload: HandlerPayload,
    signal?: AbortSignal,
) => {
